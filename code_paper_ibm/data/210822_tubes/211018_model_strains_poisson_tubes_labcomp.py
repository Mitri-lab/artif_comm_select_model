...
In each case we just need to chose "N_comms" (here as a list and the simulation for each metacommunity size will be run one after another)
...
seed = int(sys.argv[1]) # when running the script in linux and feeding the seed as external variable
#generate teh rng later within the tube
...
enter_path = "/home/pguridi/220523/"#specify path to store results of the simulation

for N_comms in [6,9,15,27,33]:
    rng = np.random.default_rng(seed)
    #CREATE THE DIRECTORY TO STORE THE RESULTS
    #here it will be num_tubes/seed/ptop 
    path = enter_path + str(N_comms) + "/" #create a folder named as the seed (to seed we could append date+time for instance)
    ...
    list_prop = ["d3", "d3"] #propagations to run, one after another
    list_treat = ["s","r"] #treatment ("s" selection, "r" random) for each of the propagations above, it should be as long as list_prop
