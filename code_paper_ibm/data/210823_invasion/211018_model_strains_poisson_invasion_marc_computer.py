...
for f_migr in [0.05, 0.15, 0.25, 0.35, 0.45, 0.55, 0.62, 0.715, 0.81, 0.905, 1]:
    rng = np.random.default_rng(seed)
    #CREATE THE DIRECTORY TO STORE THE RESULTS
    enter_path = "/home/pguridi/220522/" #specify path to store results of the simulation
    if len(str(f_migr)) >= 4:
        f_str=str(f_migr)[0:1] + "_" + str(f_migr)[2:] 
    elif len(str(f_migr)) == 1:
        f_str = str(f_migr)
        
    #crete a folder with f_inv as name
    path = enter_path + f_str + "/"
    ...
    list_prop = ["d3", "d3"] #propagation methods to use
    list_treat = ["s","r"] #treatment ("s" selection "r" random) to apply for each of the propagations above, must be of equal lenght as "list_prop"
