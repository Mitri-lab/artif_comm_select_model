...
enter_path = "/home/pguridi/220525/"#specify path to store results of the simulation

N_comms = 21#number of different communities or tubes in the simulation 

#dilution ratios, they must be higher than 0

for d_factor in [0.2,0.5,2, 5, 20]: #in order to change the actual ratios I just multiply the regular ones times a factor
    ...
    d_p = 0.05*d_factor #dilution ratio in propagule
    d_m = 0.05*d_factor #dilution ratio in migrant pool
    d_n = 0.05*d_factor #dilution ratio in no selection
    ...
    b_d_1 = int(10*d_factor) #bottleneck disassembly 1
    b_d_2 = b_d_1 #bottleneck disassembly 2 (not used, it used to simulate a second amplification due to colony growht)
        

    #CREATE THE DIRECTORY TO STORE THE RESULTS
    #here it will be num_tubes/seed/ptop 
    path = enter_path + str(d_factor) + "/" #create a folder named as the seed (to seed we could append date+time for instance)
    if os.path.exists(path):
        ...
        try: #still had to add this because of some trouble when looping on bash to run this script several times
            ...
            pass
    #now we also create a folder for the seed
    path = path + str(seed) + "/" #create a folder named as the seed (to seed we could append date+time for instance)
    if os.path.exists(path):
        pass
    else:
        try: #still had to add this because of some trouble when looping on bash to run this script several times
    ...
    list_prop = ["d3", "d3", "p", "m", "pip", "mip", "n"] #propagations to run
    list_treat = ["s", "r", "s", "s", "s", "s", "r"] #treatment ("s" selection, "r" random), for each of the propagation methods above 
    for index_prop in range(len(list_prop)):
        ...
        #generate species (only once, they should be common for all the propagation methods)
        ...
        #SPECIES COMBINATION
        if treat == list_treat[0] and prop == list_prop[0]: #generate species combinations only once so they are shared for all the propagations
        ...
        for rp in list_repeats: 
            ...
            while rd < round_lim: #For each round
                ...
                while tm < time_lim: # For each time step
                    ...
                    if rd < 3 or (rd+1)%save_periodicity == 0 or rd == (round_lim - 1):
                        weplothere = 1
                        for t in range(N_comms):
                ...
                #-------------------------------------------------
                #save initial grid
                if rd == (round_lim - 1): #note that rd has been increased just above
