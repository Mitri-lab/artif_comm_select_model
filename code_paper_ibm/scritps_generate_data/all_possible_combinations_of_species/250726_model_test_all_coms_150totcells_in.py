...
Here we run all the possible combinations of species, taking into account that all the communities initially should have a total of 150 cells.

key parameter is: in_cells0_total = 150
...
path="/mnt/c/Users/pablo/Desktop/jul2025/" #where we will store things
...
#GENERAL PARAMETERS:
total_sp = 15
N_comms = 3 #number of different communities in teh simulation (MIGHT NEED TO ADAPT WHAT TO PLOT)
...
num_repeats = 1 #each repeat is a simualtion with the number of tubes 

#PROPAGATION #we won´t use any of this here.
"""
...
per_p = 5 #(% of the cells that will be transferred by propagule)
per_m = 5 # (% of the pool/tubes_tt that will be transferred in migrant pool)
per_n = 5 # (% of the cells transferred in the no selection control)
f_inv_d = 0.25 #fraction of tubes where migration will occur in disasembly
s_d = 1 #sensitivity disassembly, from how many cells we start to detect when we disassemble
b_d_1 = 10 #bottleneck disassembly 1
b_d_2 = b_d_1 #bottleneck disassembly 2
"""


#INSIDE EACH TUBE:
#in_cells0 = 10
in_cells0_total = 150 #initial number of cells (total population adding all species)                  
...
f.write("Initial total population: "+ str(in_cells0_total) +'\n')
...
def get_cmap(n, name='hsv'):
    ...
    """
    return plt.get_cmap(name, n)#return plt.cm.get_cmap(name, n)
...
for tup in comms:
    ...
    for t in range(N_comms):
        ...
        # Number of species in the current combination ('tup')
        num_species_t = len(tup)
        
        # Make sure there are species to avoid a division by zero
        if num_species_t > 0:
            # Calculate how many base cells correspond to each species
            cells_per_species = in_cells0_total // num_species_t
            # Calculate the remainder to distribute it and make the sum exact
            remainder = in_cells0_total % num_species_t

            for i in range(num_species_t):
                cell_here = copy.deepcopy(species[tup[i]])
                
                # Assign the base initial population to each species
                initial_pop = cells_per_species
                
                # Distribute the remainder among the first 'i' species
                if i < remainder:
                    initial_pop += 1
                
                cell_here["p0"] = initial_pop
    ...
    for t in range(N_comms):
        ...
        #for ax in fig.get_axes():
            #ax.label_outer()
    #Save the plot
    plt.savefig( path + now + "/plots"+ "/com "+ str(tup) + ".png", dpi=72)#plt.savefig( path + now + "/plots"+ "/com "+ str(tup) + ".pdf")
    plt.close(fig)
    ...
    df = pd.concat([df, df_now], ignore_index=True) #df = df.append(df_now)
