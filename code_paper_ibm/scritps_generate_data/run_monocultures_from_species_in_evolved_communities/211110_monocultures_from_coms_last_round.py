...
Model to take all teh communities in teh last round of the results (here referred to as "old" grid,), and then for each 
of them run monocultures of the species they have alone. Needed for teh synergies:P

211004
 -> when restarting the seed for each repeat use rng(seed) instead of rng(repeat)
 -> ERROR--> I was taking cells always from tube one of the old grid, now I correct to take them from the corresponding tube
 -> run all the methods with one same script (still will manually change the seed)
"""
...
#props_grid = ["d2_s", "p_s", "m_s", "pip_s", "mip_s", "n_r", "d2_r", "p_r", "m_r", "pip_r", "mip_r"]
#props_grid = [ "d3_s", "d3_r", "mip_s", "mip_r", "pip_r", "p_s", "p_r", "m_s", "m_r", "n_r"]
props_grid = [ "pip_s", "d2_s"]



#prop_grid= "mip_r" #same as above but to go one by one (cause I run this script in my computer and otherwise would be too much)
#loop over different propagation methods:
for prop_grid in props_grid:
    rng = np.random.default_rng(seed)
    #CREATE THE DIRECTORY TO STORE THE RESULTS
    path_grid = "/Users/pablo/Desktop/data_mitri/210822/" #path where the results (old grid) are stored
    enter_path="/Users/pablo/Desktop/220707/" #path to store results of the simulation
    ...
    N_comms = 3 #replicates of each monoculture
    N_comms_old = 21 #number of tubes in teh simulations from where we will take the last round
    beta = 1/3 #fraction of communities to be transferred (NOT USED HERE)
    ...
    list_repeats = range(0,10) #list with the repeats to run; 
                               ...
                               # HERE CORRESPONDS TO THE REPEATS WHEN RUNING THE OLD RESULTS
    ...
    #PROPAGATION (NOT USED HERE CAUSE IT IS JUST ONE ROUND)
    ...
    save_periodicity = 1 #at which rounds we save more detailed results
    
    
    #SAVE ALL THESE CHOICES TO A .txt FILE:
    date = datetime.now()#YY_mm_dd_H_M_S
    #date.strftime("%Y_%m_%d_%H%M%S") 
    now = prop_grid # now is indeed the path to the directory, originally called now because it included
    ...
    #generate species (HERE I DON´T REALLY USE THEM THOUGH)
    ...
    for rp in list_repeats: 
        #restart the generator so repeats can be run independently
        rng = np.random.default_rng(seed)
        #in case we want to pass the repeat when calling the script from unix
        #repeat = int(sys.argv[2])
        
        os.mkdir(path + now + "/repeat" + str(rp)) #make a folder inside the seed folder with the name of the repeat
        
        #Load the corresponding old grid
        open_file = open(path_grid + str(seed) + "/" + prop_grid + "/repeat" + str(rp) + "/grid0_round_49", "rb")
        old_grid = pickle.load(open_file)
        open_file.close()
    
        for old_tube in range(N_comms_old): #so we take each tube of the ancestral community
            os.mkdir(path + now + "/repeat{}/comm{}".format(rp,old_tube))
            for old_sp in simpler_count(old_grid[old_tube]).keys(): #for each species in that community
                os.mkdir(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp))
                os.mkdir(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp) + "/per_round") #folder inside repeat to store results at some round
                                                                                                           #here I only have a round, but it is just to keep the
                ...
                for t in range(N_comms):
                    ...
                    grid.append(sp_finder(old_grid[old_tube],old_sp))
                    ...
                    if weplothere: #the round periodicity we decided above
                        ...
                        for t in range(N_comms):
                            ...
                            #for ax in fig.get_axes():
                                #ax.label_outer()
                        #Save the plot
                        plt.savefig( path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp)  + "/per_round" + "/round" + str(rd)+".pdf")
                        #I am having a problem "MemoryError: In RendererAgg: Out of memory plt.savefig" I add the next three lines and not only one to try to fix it.
                        fig.clf()
                        plt.close(fig)
                        gc.collect() 
                        ...
                        open_file = open(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp)  + "/per_round" + "/round" + str(rd), "wb")
                        ...
                        open_file = open(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp)  + "/sp_tube" + str(rd), "wb")
            ...
            #%% Section 7
                    ...
                    if rd < (round_lim - 1):
                        ...
                        if prop == "n":
                            ...
                            if extinction_checking:
                                f=open(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp)+ '/extinctions.txt','a')
                    ...
                    if rd == (round_lim - 1) or (rd+1)%save_periodicity == 0: #note that rd has been increased just above
                        ...
                        open_file = open(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp) + "/grid0_round_" + str(rd), "wb")
                        pickle.dump(grid, open_file)
                        open_file.close()
    
    #%%SECTION 9
                #STORE IN THE LAST ROUND
                df_grid.to_csv(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp) + "/df_grid.csv")
                df_sp.to_csv(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp) + "/df_sp.csv")
                df_st.to_csv(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp) +"/df_st.csv")
                df_nuts.to_csv(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp) +"/df_nuts.csv")
                df_toxs.to_csv(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp) +"/df_toxs.csv")
                df_tubes.to_csv(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp) +"/df_tubes.csv")
                #store the grid just in case
                open_file = open(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp) + "/grid100_round_" + str(rd-1), "wb")
                pickle.dump(grid, open_file)
                open_file.close()
                ...
                f=open(path + now + "/repeat{}/comm{}/mono{}".format(rp,old_tube,old_sp) + '/timer.txt','w')
