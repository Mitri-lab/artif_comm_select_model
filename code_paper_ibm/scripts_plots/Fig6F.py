...
Here, I adapt the scritp I used to plot the analysis where we change the number of tubes or communities, to now plot the analysis where we have different number of time steps.

we change the number of tubes or communities (N_comms) in the simulations and see how 
...
# path to the data
path = "/Users/pablo/Desktop/aug2025/"
#also for the regular datasets 
path21 = "/Users/pablo/Desktop/data_mitri/210822/"
...
#we change the number of time steps we run the simulations
times = ["40", "60", "80", "100", "120"]
conditions = [  "d3_s", "p_s", "m_s",  "pip_s",  "mip_s"]
#renanme conditions for the plot
conditions_rename = ["DS", "PS", "MS",  "PIS", "MIS" ] #important, same order as above

#now anyway I´ll take the last round when it went extinct (again, I might rethink about extinction condition)


path_save = "/Users/pablo/Desktop/aug2025/plots/" #path to save results
...
auc_all = {}
sp_div_all = {}
for num_tube in times:
    #well, num_tube should be time_steps_here or something, but just to adapt it fast
    deg_all[num_tube] = {}
    auc_all[num_tube] = {}
    sp_div_all[num_tube] = {}
    for seed in seeds:
        ...
        for prop in conditions:
            ...
            for rp in range(repeats):
                ...
                if num_tube != "80": 
                    df_here = pd.read_csv(path + "t" + num_tube + "/" + seed + "/" + prop + "/" + "repeat" + str(rp) + "/df_grid.csv")
                if num_tube == "80": #21 tubes is the standar used for regular simulations and data is stored in a different folder.
...
for i_p,prop in enumerate(conditions):
    ...
    # Position text inside the plot by changing the x-coordinate
    text_x_position = 45 
    plt.text(text_x_position, correct_height + 0.12, "y = " + str(round(m,5)) + "x + " + str(round(b,2)), style='italic', fontsize=10)#, bbox={'facecolor': 'red', 'alpha': 0.5, 'pad': 8}
    plt.text(text_x_position, correct_height + 0.02, "Spr rho: {}, p-val: {}".format(round(rho,5), fmt(pval)), style='italic', fontsize=10)
    i+=1
    
    
plt.savefig(path_save + "maxdeg_times{}.pdf".format(int(rd)))
plt.savefig(path_save + "maxdeg_times{}.svg".format(int(rd)))
plt.show()
...
plt.xlabel("Time_Steps",fontsize=14)
plt.ylabel("propagation",fontsize=14)
plt.savefig(path_save + "grid_median_times_{}.pdf".format(rd))
plt.savefig(path_save + "grid_median_times_{}.svg".format(rd))
