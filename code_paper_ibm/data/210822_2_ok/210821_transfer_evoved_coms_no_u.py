...
but I take the community at time step 0 (initial bacteria present there). Then I just run 10 exact replicates of this initial
grid during 25 rounds under no selection treatment. 
...
seed = 26 #this is needed for the random number generator, but also should match the seed (species set) we want to 
            #run from the general results (as it is used for the path)
...
for prop_grid_index in range(2):
    ...
    path_grid = "/Users/pablo/Desktop/data_mitri/210822/" #where are general results stored, from where we take best comms
    ...
    now = prop_grid # now is indeed the path to the directory, originalle called now because it included
