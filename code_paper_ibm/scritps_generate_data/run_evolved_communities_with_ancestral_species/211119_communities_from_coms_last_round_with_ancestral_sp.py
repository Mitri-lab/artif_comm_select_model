...
in the last round (grid0 at round 49, wich is the round number 50 if we start counting in 0), and put the same 
species in the same initial populations but replacing evolved strains by
ancestral species. Then we let this community grow for 1 round of 80 time steps.


Choose bellow the seed (speecies set) and propagations you want to run ("list_prop" and "list_treat")
...
seed = 26 # when running the script in linux and feeding the seed as external variable
...
enter_path = "/Users/pablo/Desktop/220712/" #specify path to store results of the simulation
path_grid = "/Users/pablo/Desktop/data_mitri/210822/" #we also need here the path to the old grid
...
for index_prop in range(5):
    ...
    for rp in list_repeats: 
        ...
        #load the old grid
        open_file = open(path_grid + str(seed) + "/" + now + "/repeat" + str(rp) + "/grid0_round_49", "rb")
        ...
        for t in range(N_comms):
            ...
            for old_sp,old_pop in simpler_count(old_grid[t]).items():
                cell = copy.deepcopy(species[old_sp]) 
