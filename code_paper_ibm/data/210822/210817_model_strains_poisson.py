...
enter_path = "/scratch/pguridif/210822/" #specify path to store results of the simulation
...
#PROPAGATION
list_prop = ["d", "d2", "p", "m", "pip", "mip", "n", "d", "d2", "p", "m", "pip", "mip"]
list_treat = ["s","s", "s", "s", "s", "s", "r", "r", "r", "r", "r", "r", "r"]
index_prop = int(sys.argv[2])
        ...
        if weplothere: #the round periodicity we decided above
            ...
            for t in range(N_comms):
                ...
                #for ax in fig.get_axes():
                    #ax.label_outer()
            #Save the plot
            plt.savefig( path + now + "/repeat" + str(rp)  + "/per_round" + "/round" + str(rd)+".pdf")
            plt.close(fig)
            ...
            #---------------------------------#
                ...
                for t in tubes_to_migrate:
                    ...
                    for i_i in range(num_invaders):
                        ...
                        sp_to_include = list(set(sp_in_play)-{invader}) #The added species will not be added again
            ...
            #-----------------------------------#
                ...
                for t in tubes_to_migrate:
                    ...
                    for i_i in range(num_invaders):
                        invader = rng.choice(sp_to_include) #invader is the species number
                        grid[t].extend(sample_or_amplify(st_repo[invader], b_d_1))
                        sp_to_include = list(set(sp_in_play)-{invader}) #added species are not reintroduced
                    rng.shuffle(grid[t]) 
