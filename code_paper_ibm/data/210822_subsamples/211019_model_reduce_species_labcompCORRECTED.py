...
In this version of the model, after having a maximun of species "total_sp", 
then I resample a lower number of species "subsample_size", I originally generate
the combinations for all the times we resample "N_subsamples", so we can run each sample
separatelly.

In this particualr version I adapt to run in the workstation in the lab (unix).
...
#generate rng later for each subsample

for subsample_size in [6,9,12]: #how many species per subsample
    N_subsamples = 10 #how many subsamples we generate (just the combination, how many we run is decided later in the loop when it says for subsample_index in range(x))
    ...
    #PROPAGATION
    list_prop = ["mip", "pip"] #propagation methods
    list_treat = ["s", "s"] #treatment ("s" selection "r" random for each of the propagations in the above list)
    
    
    for index_prop in range(len(list_prop)): 
    #treat = "s" #chose: random sampling control (r), or selection (s). It has no effect in baseline prop
    #prop = "d" #choose: propagule (p), migrant_pool (m) or disassembly (d), or no selection (n)
        treat = list_treat[index_prop]
        prop = list_prop[index_prop]
        
        #CREATE THE DIRECTORY TO STORE THE RESULTS 
        enter_path = "/home/pguridi/220613/" #specify path to store results of the simulation
        #Now, create a folder dor each subsample
        path = enter_path + str(seed) + "_"+str(subsample_size) +"/"
        if os.path.exists(path):
            ...
            try: #still had to add this because of some trouble when looping on bash to run this script several times
                ...
                pass
        for subsample_index in range(6):
            rng = np.random.default_rng(seed)
            path = enter_path + str(seed) + "_"+str(subsample_size) +"/" + str(subsample_index) + "/"#create a folder named as the seed (to seed we could append date+time for instance)
            if os.path.exists(path):
                pass
            else:
                try: #still had to add this because of some trouble when looping on bash to run this script several times
            ...
            list_repeats = range(0,5) #list with the repeats to run; 
            ...
            #SPECIES SUBSAMPLES
            if treat == list_treat[0] and prop == list_prop[0]:
                sp_subsamples = []
                for subsp in range(N_subsamples):
                    sp_subsamples.append(rng.choice(range(total_sp),subsample_size,replace = False))
            
            #subsample_index = int(sys.argv[3])
            #subsample_index = 2
            
            
            #SPECIES COMBINATION
            if treat == list_treat[0] and prop == list_prop[0]:
                INDEX_REP = species_combinations(subsample_size, N_spc0, N_comms, N_repeats)
            #and save them
            ...
            for rp in list_repeats: 
                ...
                for t in range(N_comms):
                    ...
                    for i, sp in enumerate(INDEX_REP[rp][t]): #Here we add the desired species
                        cell = copy.deepcopy(species[sp_subsamples[subsample_index][sp]]) 
                ...
                while rd < round_lim: #For each round
                    ...
                    if rd < (round_lim - 1):
                        ...
                        #--------------------------#
                            ...
                            #INVASION
                            tubes_to_migrate = rng.choice (N_comms,int(N_comms*f_migr))
                            ...
                            for t in tubes_to_migrate:
                                ...
                                grid[t].extend(sample_or_amplify(st_repo[invader], b_d_1)) #here st repo is always ancestral species
                                rng.shuffle(grid[t]) 
                        ...
                        #---------------------------------#
                            ...
                            for t in tubes_to_migrate:
                                ...
                                for i_i in range(num_invaders):
                                	#I am getting an error here rarely, so just to avoid it and see if I can find the problem
                                    try:
                                        invader = rng.choice(sp_to_include) #invader is the species number
                                        ...
                                        f=open(path + now + '/error_in_pis_invader' + str(rp) + "_" + str(rd) +'.txt','w')
                                        f.write("sp_to_include: "+ str(sp_to_include) +'\n')
                                        f.write("invader: " + str(invader) + "_" + str(i_i) + '\n' )
                                        f.write("repeat: " + str(rp) +'\n' )
                                        f.write("round: " + str(rd) +'\n' )
                                        f.write("time step: " + str(tm) +'\n' )
                                        f.write("tube: " + str(t) +'\n' )
                                        f.close()
                        ...
                        #  PROPAGULE  +  INVASION2 #
                        #--------------------------#
                        #in this case we assume that we know the species in the community and that only species not 
                        #present can invade (what we do in disassembly)
                        elif prop == "pi2":
                            #we reseed the cells
                            grid2 = [] #Grid to store cells 
                            group_size = int(N_comms/N_beta) 
                            ...
                            sp_we_have = []
                            for tube in grid:
                                sp_we_have.extend(simpler_count(tube).keys())
                            tubes_to_migrate = rng.choice (N_comms,int(N_comms*f_migr))
                            #we select the species that will arrive to the tube, prioritisig those not present in the metacommunity
                            for t in tubes_to_migrate:
                                ...
                                if len(sp_to_include) > 0:
                                    ...
                                    sp_in_tube = list(simpler_count(grid[t]).keys())
                        ...
                        #present can invade (what we do in disassembly)
                            ...
                            #we store the cells to transfer
                            pool = []
                            for i in selected_tubes:
                                pool.extend(grid[i[1]])
                            grid = [] #I am not using the grid anymore, so I can erase it
                            for t in range(N_comms):
                                grid.append(dilution(pool,d_m/N_beta)) #here percentages is scaled by the 
                                                                           #number of tubes we have mixed.
                        ...
                        #----------------#
                            ...
                            #UPDATE THE FOSSIL RECORD OF STRAINS
                            control_update = [0 for sp in range(total_sp)] #chech which species have been updated
                            ...
                            for i,tup in enumerate(selected_tubes):
                                D_hat.append(tup[0] * ((N_spc[i] - E[i])/N_spc[i])) #This also works if we do not constrain to 4 species
                            ...
                            for t in tubes_to_migrate:
                                ...
                                # which of the species already present in the tube will be replaced (we try to not remove those 
                                #unique species in the metacommunity)
                            ...
                            for t in range(N_comms):
                                for sp_list in putative_tubes[t]:
                                    ...
                                    if b_d_1 == b_d_2:
                                        ...
                                        grid[t].extend(sample_or_amplify(sp_list, b_d_2)) #cells are already deactivaed
                                rng.shuffle(grid[t]) 
                        ...
                        #  DISASSEMBLY_2  #
                        #-----------------#
                        #we discussed to have a disassembly where:
                                #we randomly add or remove species in an indipendent manner
                                #the number of species we add or remove is based on poission around 1
                                #we control stinction, so we keep all the species at the beginning of a round, if does not matter 
                                #if a tube has 2 or 7 species, we make sure we keep them
                            ...
                            #choose the fraction of the tubes to get a IMMIGRANT (or invader)
                        ...
                        #  DISASSEMBLY_2_OLD  #
                        #---------------------#
                        #immigration previous to emigration
                        #we discussed to have a disassembly where:
                                #we randomly add or remove species in an indipendent manner
                                #the number of species we add or remove is based on poission around 1
                                #we control stinction, so we keep all the species at the beginning of a round, if does not matter
                            ...
                            #chose the fraction of the tubes to get a IMMIGRANT (or invader)
                            tubes_to_immigrate = rng.choice (N_comms,int(N_comms*f_migr) , replace=False)
                            #select the species that will arrive to the tube, prioritisig those not present in the metacommunity
                            for t in tubes_to_immigrate:
                                num_invaders = 1 + rng.poisson(0.5)
                                for i_i in range(num_invaders):
                                    ...
                                    #and update the list traking the species that we have in the metacomunity
                                    sp_we_have.append(invader)
                            #tubes from where species will be removed (EMIGRANT)
                            tubes_to_emigrate = rng.choice (N_comms,int(N_comms*f_migr), replace = False)
                            for t in tubes_to_emigrate:
                                num_emigrants = 1 + rng.poisson(0.5)
                                for i_i in range(num_emigrants):
                        ...
                        #I bit later I also updated so there is no invasion if the 15 species are present.
                        elif prop=="d3": 
                            N_spc = [len(sp_tube[x[1]]) for x in selected_tubes] #number of species ger tube
                            E = [0 for i in N_spc] #counter of extinction for each of the selected tubes
